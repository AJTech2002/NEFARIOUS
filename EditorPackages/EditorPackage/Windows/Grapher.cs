...
public class Grapher : EditorWindow {
	...
	private void Options() {
		...
		if (GUILayout.Button ("Grid", null))
			grid = !grid;
		if (GUILayout.Button ("Indicate", null)) {
			indicate = !indicate;
		}
		if (GUILayout.Button ("Display Names", null))
			names = !names;
		if (GUILayout.Button ("Clear Graph", null))
			a.ClearAll ();
		if (GUILayout.Button ("Advanced Options", null))
			advanced = !advanced;
